...
function preflight_checks {
    ...
    if [ "$(sudo systemctl list-units --full -all -t service --no-legend | grep -F 'klipper-v24.service')" ]; then
...
function check_download {
    ...
    if [ ! -d "${FRIX_CONFIG_PATH}" ]; then
        ...
        if git -C $frixtemppath clone -b v3.0.0-alpha https://github.com/Frix-x/klipper-voron-V2.git $frixreponame; then
